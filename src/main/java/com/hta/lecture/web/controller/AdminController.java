...
public class AdminController {
	...
	public String adminCouponList(@RequestParam(name = "page", required = false, defaultValue = "1") int page,
			ClassCriteria criteria, Model model) {
		...
		// 검색조건(value)과 조회범위(beginIndex, endIndex)가 포함된 Criteria를 서비스에 전달해서 데이터 조회
		List<CouponDto> coupons = adminService.getAllCoupon();
		
		model.addAttribute("coupons", coupons);		
		model.addAttribute("pagination", pagination);
		
		return "/admin/adminCoupon"; // classList.jsp
	}
